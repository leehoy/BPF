...
__global__ void flat_distance_backproj_arb_pi(float* Dest, float* Src1,float* Src2,float* x_plane,float* y_plane,float* z_plane,float* u_plane,float* v_plane,float* angular_weight,float* params){
    int x=blockDim.x*blockIdx.x+threadIdx.x;
    int y=blockDim.y*blockIdx.y+threadIdx.y;
    int tid=y*gridDim.x*blockDim.x+x;
    float dx=params[0],dy=params[1],dz=params[2];
    ...
    float slope_min,slope_max;
    int s_index_u,e_index_u,s_index_v,e_index_v;
    float weight1=0.0,weight2=0.0,InterpWeight1=0.0,InterpWeight2=0.0,angle_weight=0.0;
    ...
    if(tid<N){
        ...
        ix=(int)(tid-iz*nx*ny-iy*nx);
        yc=fy(x_plane[ix],y_plane[iy],z_plane[iz],angle1,angle2);
        xc=fx(x_plane[ix],y_plane[iy],z_plane[iz],angle1,angle2);
        zc=fz(x_plane[ix],y_plane[iy],z_plane[iz],angle1,angle2);
        ...
        InterpWeight1=(D*D)/((R-xc*0.0-yc*1.0-zc*0.0)*(R-xc*0.0-yc*1.0-zc*0.0));
        InterpWeight2=sqrtf((xc-SourceX)*(xc-SourceX)+(yc-SourceY)*(yc-SourceY)+(zc-SourceZ)*(zc-SourceZ));
        angle_weight=angular_weight[tid];
        for(k=s_index_v;k<=e_index_v;k++){
            if(k>=0 && k<=nv-1){
                if(s_index_v==e_index_v){
                    weight1=1.0f;
                ...
                for(l=s_index_u;l<=e_index_u;l++){
                    if(l>=0 && l<=nu-1){
                        ...
                        atomicAdd(&Dest[tid],Src1[k*nu+l]*weight1*weight2*angle_weight*InterpWeight1+Src2[k*nu+l]*weight1*weight2*angle_weight/InterpWeight2);
                    }
                }
            }
        }
